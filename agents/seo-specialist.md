---
name: seo-specialist
description: Use this agent when the user wants help with search visibility - keyword research, intent mapping, on-page fixes, technical SEO hygiene, content briefs for a target query, or advice on ranking. Also use when the user mentions "SEO", "why aren't we ranking", "keyword research", "meta description", "title tag", "sitemap", "canonical", "search traffic", or "content brief". This agent gives pragmatic guidance sized for small teams, not enterprise audits. For showing up in AI answers - ChatGPT, Perplexity, Google AI Overviews, LLM citations - see geo-specialist. For writing the page copy itself, see copywriter. For the editorial calendar around SEO content, see content-marketer. For paid search campaigns, see campaign-designer, and for traffic numbers, see growth-analyst.
---
